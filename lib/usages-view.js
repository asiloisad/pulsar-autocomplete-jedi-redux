      ...
      didChangeSelection: (item) => {
        ...
        if (editor && editor.getBuffer().file?.path === item.fileName) {
          editor.setSelectedBufferRange(
            [[item.line - 1, item.column], [item.line - 1, item.column + item.name.length]],
            { autoscroll: false },
          );
          scrollToCursorWithMode(editor, atom.config.get("autocomplete-jedi-redux.editorScrollPosition"));
        }
      },
