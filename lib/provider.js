...
module.exports = {
  ...
  noExecutableError(error) {
    ...
    atom.notifications.addWarning("autocomplete-jedi-redux unable to find python binary.", {
...
to hide such errors in future. Traceback output:`,
          ...
          atom.notifications.addError("autocomplete-jedi-redux traceback output:", {
            detail: `${data}`,
            dismissable: true,
          });
        }
  ...
  constructor() {
    ...
    this.disposables.add(
      atom.config.observe("autocomplete-jedi-redux.pythonCommand", (value) => {
      ...
      atom.config.observe("autocomplete-jedi-redux.enableCompletion", (value) => {
    ...
    try {
      this.triggerCompletionRegex = RegExp(atom.config.get("autocomplete-jedi-redux.triggerRegex"));
    } catch (err) {
      atom.notifications.addWarning(
...
Falling back to default value.`,
      ...
      atom.config.set("autocomplete-jedi-redux.triggerRegex", "([\. ]|[a-zA-Z_][a-zA-Z0-9_]*)");
    ...
    atom.commands.add("atom-workspace", "autocomplete-jedi-redux:add-roots-to-extra-paths", () => {
      const current = atom.config
        .get("autocomplete-jedi-redux.extraPaths")
        .split(";")
        .map((p) => p.trim())
        .filter(Boolean);
      let added = 0;
  ...
  goToDefinition(editor, bufferPosition) {
    ...
    return this.getDefinitions(editor, bufferPosition).then((results) => {
      if (results.length === 1) {
        ...
        atom.workspace.open(fileName, { pending: true }).then((ed) => {
          ed.setCursorBufferPosition([line, column], { autoscroll: false });
          scrollToCursorWithMode(ed);
        });
        return;
      }
